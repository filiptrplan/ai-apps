...
function EditableNum({ value, onChange, min = 1, max = 999, suffix = "", step }) {
  ...
  return (
    <div style={s.numRow}>
      <button style={s.numBtn} onClick={() => onChange(Math.max(min, value - getStep(value, -1)))}>−</button>
      {editing ? (
        <input
          ref={inputRef}
          style={s.numInput}
        ...
        <span style={s.numVal} onClick={() => { setDraft(String(value)); setEditing(true); }}>
          {value}{suffix}
        </span>
      )}
      <button style={s.numBtn} onClick={() => onChange(Math.min(max, value + getStep(value, 1)))}>+</button>
    </div>
  );
}
...
function IntervalTimerApp() {
  ...
  // Ref-based tick (avoids stale closures in setInterval)
  ...
  const skipPhase = () => {
    if (phaseRef.current === "rest") {
      ...
      // Skip work -> rest, or done if last set
      if (currentSetRef.current >= configRef.current.totalSets) {
        clearInterval_();
        phaseRef.current = "done";
        setPhase("done");
  ...
  const downloadExport = () => {
    ...
    a.href = url;
    a.download = `interval-timer-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  ...
  return (
    <div style={s.root}>
          ...
          {isIdle && (
            <div style={s.config}>
              <div style={s.field}>
                ...
                <EditableNum value={workSec} onChange={setWorkSec} suffix="s" />
              </div>
              <div style={s.field}>
                <label style={s.label}>Rest</label>
                <EditableNum value={restSec} onChange={setRestSec} suffix="s" />
              </div>
              <div style={s.field}>
                <label style={s.label}>Sets</label>
                <EditableNum value={totalSets} onChange={setTotalSets} min={1} max={99} step={1} />
              </div>
            </div>
          )}
          ...
          <div style={{ ...s.timerBox, background: phaseBg }}>
            {isIdle && (
              <>
                ...
                <div style={s.timerSub}>
                  {totalSets} sets &middot; {formatTime(workSec)} work &middot; {formatTime(restSec)} rest
                </div>
                <div style={s.timerSub2}>
                  Total: {formatTime(totalSets * workSec + (totalSets - 1) * restSec)}
                </div>
              </>
            )}
            {running && (
              <>
                ...
                <div style={s.timerSub}>
                  Set {currentSet} / {configRef.current.totalSets}
                </div>
            ...
            {isDone && (
              <>
                ...
                <div style={s.timerSub}>
                  {configRef.current.totalSets} sets completed
                </div>
              </>
            )}
          </div>
          ...
          <div style={s.controls}>
            ...
            {running && (
              <>
                ...
                <button style={s.skipBtn} onClick={skipPhase}>Skip</button>
                <button style={s.stopBtn} onClick={stopTimer}>Stop</button>
              </>
            )}
            {isDone && (
              <button style={s.startBtn} onClick={stopTimer}>Reset</button>
            )}
          </div>

          {/* Quick load presets */}
          {isIdle && presets.length > 0 && (
            <div style={s.quickPresets}>
              <div style={{ ...s.label, marginBottom: 8 }}>Quick load</div>
              <div style={s.presetChips}>
                {presets.map(p => (
                  <button key={p.id} style={s.chip} onClick={() => loadPreset(p)}>
                    {p.name}
                  </button>
                ))}
              </div>
      ...
      {tab === "Presets" && (
        <div style={s.page}>
          <div style={s.presetForm}>
            <input style={s.input} placeholder="Preset name"
              value={presetName} onChange={e => setPresetName(e.target.value)}
              onKeyDown={e => e.key === "Enter" && savePreset()} />
            <button style={s.saveBtn} onClick={savePreset} disabled={!presetName.trim()}>Save current</button>
          </div>
          <div style={s.configSummary}>
            Saving: {formatTime(workSec)} work / {formatTime(restSec)} rest / {totalSets} sets
          </div>

          {presets.length === 0 && <p style={s.empty}>No presets yet. Configure your timer and save one.</p>}
          {presets.map(p => (
            <div key={p.id} style={s.listItem}>
              <div style={s.listMain} onClick={() => loadPreset(p)}>
                <div style={s.listTitle}>{p.name}</div>
                <div style={s.listMeta}>
                  {formatTime(p.workSec)} / {formatTime(p.restSec)} &middot; {p.sets} sets
                </div>
              </div>
              <button style={s.deleteBtn} onClick={() => deletePreset(p.id)}>&times;</button>
            </div>
          ))}
          ...
          <div style={s.exportSection}>
            ...
            <div style={s.exportHint}>Move presets and history between devices.</div>
          </div>
        </div>
      )}
      ...
      {tab === "History" && (
        <div style={s.page}>
          {history.length > 0 && (
            <button style={s.clearBtn} onClick={clearHistory}>Clear all</button>
          )}
          {history.length === 0 && <p style={s.empty}>No completed workouts yet.</p>}
          {history.map(h => (
            <div key={h.id} style={s.listItem}>
              <div style={s.listMain}>
                <div style={s.listTitle}>{formatDate(h.date)}</div>
                <div style={s.listMeta}>
                  {formatTime(h.workSec)} / {formatTime(h.restSec)} &middot; {h.sets} sets
                  &middot; {formatTime(h.sets * h.workSec + (h.sets - 1) * h.restSec)} total
                </div>
              </div>
            ...
            {transferMode === "export" && (
              <>
                ...
                <div style={s.modalActions}>
                  <button style={{ ...s.exportBtn, flex: 1 }} onClick={copyExport}>
                    {copied ? "Copied!" : "Copy"}
                  </button>
                  <button style={{ ...s.exportBtn, flex: 1 }} onClick={downloadExport}>
                    Download
                  </button>
                </div>
              </>
            ...
            {transferMode === "import" && (
              <>
                ...
                <div style={s.modalActions}>
                  ...
                  <button style={{ ...s.exportBtn, flex: 1 }} onClick={() => fileInputRef.current?.click()}>
                    From file
                  </button>
...
const s = {
  ...
  tabActive: {
    color: "#E8E8E8", borderBottomColor: "#E8E8E8",
  },
  page: { padding: "20px 16px" },
  config: {
    display: "flex", gap: 8, marginBottom: 24, minWidth: 0,
  },
  ...
  numBtn: {
    ...
    background: "#1A1A1A", color: "#CCC", fontSize: 18, cursor: "pointer",
    display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
  },
  numVal: {
    fontSize: 17, fontWeight: 600, fontVariantNumeric: "tabular-nums",
    minWidth: 40, textAlign: "center", cursor: "pointer",
    padding: "4px 2px", borderRadius: 6, borderBottom: "1px dashed #444",
  },
  numInput: {
    width: 52, textAlign: "center", fontSize: 17, fontWeight: 600,
  ...
  phaseLabel: {
    fontSize: 14, fontWeight: 700, letterSpacing: "0.12em", marginBottom: 8,
  },
  timerDigits: {
    fontSize: 72, fontWeight: 200, fontVariantNumeric: "tabular-nums",
  ...
  startBtn: {
    flex: 1, maxWidth: 200, padding: "14px 0", borderRadius: 10, border: "none",
    background: "#E8553A", color: "#fff", fontSize: 16, fontWeight: 600, cursor: "pointer",
  },
  pauseBtn: {
  ...
  saveBtn: {
    ...
    background: "#333", color: "#E8E8E8", fontSize: 13, fontWeight: 600, cursor: "pointer",
    whiteSpace: "nowrap",
  },
  ...
  deleteBtn: {
    ...
    background: "transparent", color: "#555", fontSize: 20, cursor: "pointer",
  },
  clearBtn: {
  ...
  exportSection: {
    marginTop: 32, paddingTop: 20, borderTop: "1px solid #222",
  },
...
ReactDOM.createRoot(document.getElementById("app")).render(<IntervalTimerApp />);
